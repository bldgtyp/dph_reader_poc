...
| 🔴 | **`glued_to` is the ONLY thing that identifies a window host.** `cuts_opening?` is a definition capability (true on all 46); `loops.size > 1` is about *modelled* holes and is true on only **2 of 16** real hosts | [`DESIGNPH_DATA_MODEL.md`](DESIGNPH_DATA_MODEL.md) §5.0 |
