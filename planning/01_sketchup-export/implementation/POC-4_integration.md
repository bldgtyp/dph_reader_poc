...
| §3's byte-identity check runs on one extraction | It has **five** real ones now, in `pocs/01_sketchup-export/_private/fixtures/`. Run it on all five — Adelphi masked three separate bugs in POC-2's harness by being the simplest model in the corpus |
...
5. Ruby: save-file dialog (default `<model>_export/<model>.hbjson`), write HBJSON + `.report.json`
   ...
   under `pocs/01_sketchup-export/_private/` (gitignored — client data), per the overview §8 rule.
...
2. **[Ed] End-to-end run** on Adelphi COPY: export with the extraction toggle on; save all three
   output files into `pocs/01_sketchup-export/_private/` for comparison. The identity claims, stated precisely:
