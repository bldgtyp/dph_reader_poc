...
The POC proved every downstream step (`../01_sketchup-export/implementation/RESULTS/POC-5_results.md`): 545/545 classified faces,
...
- **The corpus**: 14 models baselined key-by-key in
  [`../01_sketchup-export/feasibility/RESULTS/PHASE-0_corpus-baseline.md`](../01_sketchup-export/feasibility/RESULTS/PHASE-0_corpus-baseline.md).
- **Five live captures** (contract v2 JSON) in `pocs/01_sketchup-export/_private/fixtures/` — what the Ruby collector
  actually read inside SketchUp, from five real projects. **The ground truth.**
- **The harness and translator, unchanged**: `pocs/01_sketchup-export/tools/check_extraction.py`,
  `validate_output.py`, `byte_identity.py`'s canonicalisation discipline, and
  `pocs/01_sketchup-export/py/dph_translator`.
...
strengthened 2026-08-28). This repo is the *cleaned export*: `pocs/01_sketchup-export/_private/` is absent,
`planning/01_sketchup-export/feasibility/RESULTS/baselines/` carries no `corpus_baseline.json` (phase-2/3 artifacts only), and
`corpus/adelphi/` holds only its `.index.md` — **no primary `.skp` at all**. The
canonical copies live in `~/Desktop/dph_plus_testing` (`pocs/01_sketchup-export/_private/fixtures/` — the five
captures + MANIFEST; `planning/01_sketchup-export/feasibility/RESULTS/baselines/corpus_baseline.json`; the primary corpus files);
...
- **L2 — Reframe the AGPL question for server-side use.** The filed question
  ([`../01_sketchup-export/feasibility/RESULTS/PHASE-3_licence-question.md`](../01_sketchup-export/feasibility/RESULTS/PHASE-3_licence-question.md)) is about
