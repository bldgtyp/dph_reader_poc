...
This is the cleaned export; `planning/01_sketchup-export/feasibility/RESULTS/baselines/corpus_baseline.json` does not exist here
(the `baselines/` dir only holds phase 2/3 artifacts), and `pocs/01_sketchup-export/_private/fixtures/` is absent
entirely. The harness hardcodes the baseline path (`pocs/01_sketchup-export/tools/check_extraction.py:36`), so H3 dies
