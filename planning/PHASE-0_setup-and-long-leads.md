...
```
planning/RESULTS/          # phase results land here, one file per phase
```
...
for f in ../_adephi_st_example_files/adelphi-designph.skp \
         ~/Dropbox/bldgtyp/*/08_DesignPH/*.skp ; do
  uv run tools/skp_attr_dump.py "$f" -d DesignPH_dict > "../planning/RESULTS/baseline_$(basename "$f" .skp).txt" 2>&1
done
```
...
`_misc_test_files/designph_test*.skp` models (2.4.0 BETA / 2.2.29), the only 2.4-generation samples
we have.
...
`_adephi_st_example_files/adelphi-honeybee-json.hbjson` exists: honeybee schema **1.53.1**, Meters,
