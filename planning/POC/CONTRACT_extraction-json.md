...
- **Synthetic fixtures** (`poc/py/tests/fixtures/synthetic/`) are hand-written JSONs of this shape,
...
- **Real fixtures** (`poc/_private/fixtures/`, POC-2 deliverable) are collector output captured from
  ...
  evidence). They contain client data — `poc/_private/` is gitignored, manifest-tracked.
...
(`poc/_private/fixtures/adelphi-designph_COPY.extraction.json`). Detail in
