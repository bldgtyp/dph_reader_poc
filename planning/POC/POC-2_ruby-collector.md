...
**Builds:** `poc/ext/dph_plus_poc/collector.rb` — walks a live designPH model and emits the
...
- `poc/ext/tests/test_collector.rb` with a stub entity tree: transform accumulation (nested,
...
1. **[Ed] One batched console-script session:** a standalone
   `poc/ext/tests/run_collector_console.rb` that loads `collector.rb`, loops over every model in a
...
2. **Count reconciliation, automated:** a `poc/tools/check_extraction.py` (PEP 723, `uv run`) that
...
3. **Fixture capture:** the verified outputs land in `poc/_private/fixtures/` (gitignored — client
