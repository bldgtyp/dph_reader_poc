...
**Builds:** `poc/py/dph_translator/` — a pure-Python package that turns an
...
poc/py/
  ...
  tests/               # pytest; fixtures/synthetic/ (checked in); real fixtures load from
                       # poc/_private/fixtures/ (gitignored — client data) and skip when absent
...
1. **Tier-2 pass-through (Adelphi):** `assemblies_ud.U_value` per referenced assembly vs
   `planning/RESULTS/phpp/phpp_u-values_assemblies.csv`, **joined by assembly name/description,
