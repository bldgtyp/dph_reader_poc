...
`poc/_private/` is gitignored and reserved for POC-2's real-corpus captures. It does not exist yet —
...
| `vendor/` "generated by `vendor_payload.py`" | Two tiers: `poc/vendor/` (download cache) → `poc/ext/dph_plus_poc/vendor/` (staged, trimmed) | Same shape as the spike. It is what lets `verify_in_chrome.py` detect a staged tree that has drifted from source |
...
- **One place for each fact that two things depend on.** The wheel pins live only in
  `poc/py/pyproject.toml` — the Makefile installs from it and `vendor_payload.py` parses it, so the
