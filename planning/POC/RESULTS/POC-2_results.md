...
`uv run poc/tools/check_extraction.py` → **PASS, 23 checks**, including:
...
Fixture banked at `poc/_private/fixtures/` with `MANIFEST.md`.
...
`uv run poc/tools/check_extraction.py ~/Desktop/dph_poc_copies/*.extraction.json` → **5 of 5 PASS.**
...
| File | What it does |
|---|---|
| `poc/ext/dph_plus_poc/collector.rb` | The read layer. One recursive walk → contract-**v2** extraction JSON |
| `poc/ext/tests/sketchup_stub.rb` | A stub SketchUp API — entities, `Geom::Transformation` as real 4×4 column-major maths, `UI` |
| `poc/ext/tests/test_collector.rb` | **77 checks** over that stub. The suite that keeps the Ed budget at two sessions |
| `poc/ext/tests/run_collector_console.rb` | `Dph.here` captures the open model **copy** and `Dph.status` says what is left; proves `model.modified?` never changed. ⚠ Batching is *not* the supported path — `Sketchup.open_file` does not switch documents on macOS (§8.1) — and ⚠ **the destination is never derived from `model.path`** (§8.2) |
| `poc/ext/tests/inspect_window_console.rb` | `DphWin.inspect_one` — dumps a window definition's nesting and its local/parent/world transforms. Written to answer W-1, which it did |
| `poc/tools/check_extraction.py` | Reconciles a capture against the Phase 0/1 offline baselines. **POC-2's gate, as a tool** |
| `poc/py/tests/test_check_extraction.py` | **21 tests** of that harness, against the real baseline and synthetic captures |
...
out of `planning/RESULTS/baselines/corpus_baseline.json` arithmetically**, so none of them is
...
| Real fixtures landed in `poc/_private/` with a manifest | ✅ all five, `MANIFEST.md` current |
