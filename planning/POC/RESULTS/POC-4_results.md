...
| | |
|---|---|
| `cd poc && make ci` | ✅ green — 174 pytest cases, **four** Ruby suites, schema gate, real Chromium 88 |
| `cd poc && make identity` | ✅ **5 of 5** fixtures agree across CPython 3.11 and Chromium 88 — *canonically*, see §3 |
...
|---|---|
| Version gate (the §1 decision table) | `poc/ext/dph_plus_poc/gate.rb` — a new module, pure functions over plain data |
...
| Byte-identity across hosts | `poc/tools/byte_identity.py`, `make identity` |
...
`planning/POC/RESULTS/baselines/` **by default** — a committed directory — and that baseline embeds
...
The rule it breaks is already written down (`poc/_private/MANIFEST.md`: client data, gitignored,
