...
Five jobs:
  ...
  2. zip `pocs/01_sketchup-export/py/dph_translator/` into the payload, so the runtime
     installs it by the **same
...
Usage:
    uv run pocs/01_sketchup-export/tools/build_rbz.py
    uv run pocs/01_sketchup-export/tools/build_rbz.py --check       # is the staged payload current?
    uv run pocs/01_sketchup-export/tools/build_rbz.py --install     # also copy into SketchUp 2022
"""
...
def check_staged() -> int:
    ...
    if recorded != current:
        ...
        return 1
    print(f"  ✓ staged translator matches pocs/01_sketchup-export/py/dph_translator ({current[:12]})")
    return 0
