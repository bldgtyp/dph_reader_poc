...
Usage:
    uv run pocs/01_sketchup-export/tools/byte_identity.py   # every fixture in _private/fixtures
    uv run pocs/01_sketchup-export/tools/byte_identity.py --cpython-only        # skip the browser, ~2 s
    uv run pocs/01_sketchup-export/tools/byte_identity.py FIXTURE.extraction.json ...
"""
...
def under_chromium(extraction: Path, destination: Path, chrome: Path | None) -> Leg:
    ...
    # `planning/01_sketchup-export/implementation/RESULTS/baselines/`, which is **committed** —
    # and its baseline embeds the first
...
def report(outcomes: list[Outcome]) -> bool:
    ...
    if not outcomes:
        print("  no fixtures — _private/fixtures is gitignored client data; capture it first")
