...
Downloads, verifies and unpacks everything the extension needs to run **offline**:
    ...
    pocs/01_sketchup-export/vendor/pyodide/            unpacked `pyodide-core-<ver>.tar.bz2` (the runtime)
    pocs/01_sketchup-export/vendor/wheels/*.whl        the 8 wheels of PHASE-2 results §2.6
    pocs/01_sketchup-export/vendor/manifest.json       what was fetched, its sha256, and its size

`pocs/01_sketchup-export/vendor/` is the download cache; `build_rbz.py` stages a trimmed copy into
`pocs/01_sketchup-export/ext/dph_plus_poc/vendor/`, which is what actually ships. Both are
...
Usage:
    uv run pocs/01_sketchup-export/tools/vendor_payload.py               # required payload (8 wheels)
    uv run pocs/01_sketchup-export/tools/vendor_payload.py --with-phx    # + the PHX write-path stretch goal
    uv run pocs/01_sketchup-export/tools/vendor_payload.py --check
"""
...
#: The payload, pinned — read from **`pocs/01_sketchup-export/py/pyproject.toml`, the single
#: source of truth**.
#:
...
def required_wheels() -> dict[str, str]:
    """The `name == version` pins from `pocs/01_sketchup-export/py/pyproject.toml`, in declaration order."""
    pins: dict[str, str] = {}
